...
namespace SistemaCambio.Views
{
    public partial class VentaWindow : Window
    {
        ...
        public VentaWindow()
        {
            ...
            var viewModel = new VentaViewModel(apiClient, offlineService, dialogService);
            ...
            viewModel.OperacionGuardada += (operacionId, isOffline, mensaje) =>
            {
                if (isOffline)
                    ...
                    NotificationService.OperacionGuardada("Venta", operacionId);
            };
            ...
            {
                if (e.PropertyName == nameof(VentaViewModel.CuentasDebitar))
                    cmbCuentaDebitar.ItemsSource = viewModel.CuentasDebitar;
                if (e.PropertyName == nameof(VentaViewModel.CuentaDebitar))
                    CuentaAutoComplete.Seleccionar(cmbCuentaDebitar, viewModel.CuentaDebitar);
                if (e.PropertyName == nameof(VentaViewModel.CuentasAcreditar))
                    cmbCuentaAcreditar.ItemsSource = viewModel.CuentasAcreditar;
                if (e.PropertyName == nameof(VentaViewModel.CuentaAcreditar))
                    CuentaAutoComplete.Seleccionar(cmbCuentaAcreditar, viewModel.CuentaAcreditar);
                if (e.PropertyName == nameof(VentaViewModel.MonedaNombre))
                {
            ...
            cmbCuentaDebitar.LostFocus += (_, _) =>
            {
                if (DataContext is VentaViewModel vm)
                    vm.CuentaDebitar = CuentaAutoComplete.ObtenerSeleccion(cmbCuentaDebitar);
            };
            cmbCuentaAcreditar.LostFocus += (_, _) =>
            {
                if (DataContext is VentaViewModel vm)
                    vm.CuentaAcreditar = CuentaAutoComplete.ObtenerSeleccion(cmbCuentaAcreditar);
            };
        }
        ...
        {
            if (DataContext is not VentaViewModel vm || vm.MonedaSeleccionada == null) return;
            var match = MonedaSearch.BuscarPorNombre(txtMonedaNombre.Text ?? "", vm.MonedasDisponibles);
            if (match == null)
            {
                _actualizandoDesdeCombo = true;
