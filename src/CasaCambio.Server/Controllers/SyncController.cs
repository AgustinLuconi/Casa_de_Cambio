    ...
    public IActionResult Push([FromBody] SyncPushRequest request)
    {
        ...
        foreach (var op in request.Operaciones.OrderBy(o => o.FechaCreacionLocal))
        {
            try
            {
                ...
                {
                    result = _operacionService.GuardarOperacion(op.TipoOperacion, op.CuentaOrigenId, op.CuentaDestinoId, op.MonedaOrigen, op.MonedaDestino, op.MontoOrigen, op.MontoDestino, op.CotizacionAplicada, op.Observaciones, op.LocalId);
                    if (result.Exitoso)
                    {
